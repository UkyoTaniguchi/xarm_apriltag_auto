...
def load_from_tag_pose_file(path, wanted_tag_id=2):
    """
    tag_pose.txt から
    ...
    cam1_idx = None
    for i, ln in enumerate(lines):
        if ln.startswith(f"cam_1_color_optical_frame (Tag {wanted_tag_id}) w.r.t world:"):
            cam1_idx = i
            break
    if cam1_idx is None:
...
def broadcast_transform():
    ...
    tag_id   = rospy.get_param("~tag_id", 2)
    ...
    local_offset = np.array([-0.02, 0.04, -0.0525])  # cam_3_link基準の平行移動
    ...
    # cam_4_link ← cam_1_color_optical_frame の変換
    T_c1_c4_pos  = final_translation
    T_c1_c4_quat = corrected_quat

    # 回転の合成（クォータニオン積）
    T_w_c4_quat = tft.quaternion_multiply(T_w_c1_quat, T_c1_c4_quat)

    # 平行移動の変換：cam_1→cam_4 のベクトルを world の回転で変換
    R_w_c1 = tft.quaternion_matrix(T_w_c1_quat)[:3, :3]
    T_c4_offset_world = R_w_c1 @ T_c1_c4_pos

    # 最終的な平行移動（world→cam_4）
    T_w_c4_pos = T_w_c1_pos + T_c4_offset_world

    rospy.loginfo(f"[OK] Loaded from {tag_file} (Tag ID: {tag_id})")
    rospy.loginfo(f"T_w_c4_pos = {T_w_c4_pos}")
    rospy.loginfo(f"T_w_c4_quat = {T_w_c4_quat}")
